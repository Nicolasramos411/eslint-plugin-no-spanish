...
// franc-min@6+ is ESM-only, which `require()` can't load synchronously —
// this rule runs from an ESLint visitor and a worker thread, neither can
// `await` a dynamic import. `lib/vendor/franc-min.cjs` is a CJS bundle of
// franc-min@6 built by `scripts/build-vendor.js`; using it instead of a
// live franc-min@5 dependency also drops the `trim` ReDoS CVE
// (GHSA-w5p7-h5w8-2hfq) that came through franc-min@5 > trigram-utils@1.
const { franc } = require('../vendor/franc-min.cjs');
